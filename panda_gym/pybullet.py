...
class PyBullet:
    ...
    def get_bodies_id(self, body_name) -> Optional[int]:
        """Get the body unique id."""
        return self._bodies_idx.get(body_name, None)

    def get_bodies_name(self, body_id) -> Optional[str]:
        """Get the body unique id."""
        for i, j in self._bodies_idx.items():
            if j == body_id:
                return i
        return None